...
public void distort(int mult){
    ...
    int x2 = round(x1 + random(-80 * mult * strokeWeightVolume, 80 * mult * strokeWeightVolume));
    int y2 = round(y1 + random(-80 * mult * strokeWeightVolume, 80 * mult * strokeWeightVolume));
  
    int w = (int)(75 * mult * strokeWeightVolume);
    int h = (int)(75 * mult * strokeWeightVolume);
...
public void draw3d() {
  ...
  stroke(random(0, 1) * (255  * strokeWeightVolume), random(0,1) * (255 * strokeWeightVolume), random(0,1) * (255 * strokeWeightVolume));
  ...
  for (int gridY=-width/4; gridY<width * 1.15f; gridY+=width/tileCount) {
    for (int gridX=-height/4; gridX<height * 1.15f; gridX+=height/tileCount) {
      ...
      float diameter = dist(xMusicFollow * 2, yMusicFollow * 2, gridX, gridY);
      diameter = diameter/max_distance * 10;
      pushMatrix();
        rect(gridX + diameter * 3, gridY + diameter * 3, diameter, diameter);    //// also nice: ellipse(...)
      popMatrix(); 
    }
  }
}
